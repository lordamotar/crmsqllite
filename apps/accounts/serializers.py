    ...
    def validate(self, attrs):
        if attrs['password'] != attrs['password_confirm']:
            raise serializers.ValidationError(
                {"password": "Поля пароля не совпали."}
            )
        return attrs
...
class UserLoginSerializer(serializers.Serializer):
    ...
    email = serializers.CharField()  # Может быть email или username
    ...
    def validate(self, attrs):
        ...
        if email and password:
            # Поскольку USERNAME_FIELD = 'email', используем email напрямую
            user = authenticate(
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            'role', 'full_name', 'avatar', 'bio', 'created_at', 
...
class ChangePasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if attrs['new_password'] != attrs['new_password_confirm']:
            raise serializers.ValidationError(
                {'new_password': 'Поля пароля не совпадают.'}
            )
        return attrs
